...
class EmailProvider {
  ...
  Future<void> sendAll({String? to, required String subject, required String body}) async {
    print("Sending email to $to: $subject");
  }

  Future<void> sendMixed(Object here, {required String to, required String subject}) async {
    print("Sending email to $to: $subject");
  }

  Future<String?> sendPos(Object when, [String? to, String? subject, String? body]) async {
    print("Sending email to $to: $subject");
    return null;
  }
}
...
final class AsyncConfig {
  @Pod(value: "testingAsync")
  Future<ValidationError> getValidation() async => ValidationError(field: "", message: "", rejectedValue: "", validationType: "");

  @Pod(value: "testingAsyncGeneric")
  Future<List<String>> getGeneric() async => ["Hello"];
}
...
@Configuration()
@ComponentScan(
  basePackages: ['package:example/test.dart.services', 'package:example/test.dart.repositories'],
  includeFilters: [
    ComponentScanFilter(FilterType.ANNOTATION, classes: [ClassType<Service>()]),
  ],
  excludeFilters: [
    ComponentScanFilter(FilterType.REGEX, pattern: '.*Internal.*'),
  ],
)
